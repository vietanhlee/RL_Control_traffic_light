        ...
        try:
            ...
            async with engine_lock:
                ...
                if active_connections and snapshot is not None:
                    rl_state_dict = asdict(engine.rl_state())
            
            # Nhường Event Loop cho các API requests (ví dụ /api/v1/states)
            ...
            if active_connections and snapshot is not None and rl_state_dict is not None:
                ...
                for intersection_id, intersection in engine.intersections.items():
                    ...
                    for incoming in intersection.incoming_nodes:
                        metrics = engine.collector.snapshot_direction((intersection_id, incoming), snapshot.now)
                        ...
                        direction_count += 1
                        if snapshot.light_states.get((intersection_id, incoming)) == LightColor.RED:
                            if q_val > 0.0:
                                red_pressure += q_val
                    if direction_count:
                    ...
                    switched = (snapshot.now - engine.last_switch_time.get(intersection_id, -9999.0)) < 1.1
